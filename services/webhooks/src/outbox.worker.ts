...
export class OutboxWorker implements OnModuleInit {
  private logger = new Logger('WebhooksOutboxWorker');
  ...
  async onModuleInit() {
    ...
    setInterval(
      () =>
        void this.drain().catch((e) => this.logger.warn(`outbox drain error: ${e?.message || e}`)),
      intervalMs,
    );
  }

  private async drain(limit = 50) {
    const items = await (this.prisma as any).outbox.findMany({
    ...
    for (const it of items) {
      try {
        ...
        await (this.prisma as any).outbox.update({
          where: { id: it.id },
          data: { status: 'published', attempts: (it.attempts || 0) + 1, publishedAt: new Date() },
        });
        eventsPublishedTotal.inc({ type: it.type });
      } catch (e) {
        await (this.prisma as any).outbox.update({
          where: { id: it.id },
          data: { attempts: (it.attempts || 0) + 1 },
        });
      }
    }
  }
